...
<details>
  ...
  <div><strong>Technical Response:</strong> An array is a special data structure that is used to store an ordered collection of values. Arrays are ordered with numbered indexes start at zero and encapsulated in square brackets. To access the values, you must use the variable and an assigned index number to extract the value.<br /><br />
  </div>
  </div>
</details>
...
<details>
  ...
  <div><strong>Technical Response:</strong> The two common ways to loop an array is using the for and for…of loops. Both looping structures are optimized for performance with arrays. In addition, to these two loops the array forEach method is available, but it is known to cause bottlenecks.<br /><br />
  </div>
  </div>
</details>
