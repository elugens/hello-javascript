...
**Question:** **Can you briefly explain the purpose of the XMLHttpRequest object?**
...
**Question:** **What are the two modes of operation that an XMLHttpRequest uses?**

**Interview Answer:** XMLHttpRequest use both synchronous and asynchronous modes of operation. This can be invoked, synchronously or asynchronously, using the XMLHttpRequest open method. The XMLHttpRequest method open() initializes a newly-created request, or re-initializes an existing one. If you want to run your request in async then the async parameter should be set to true, otherwise false for the default behavior. It should be noted that open call, contrary to its name, does not open the connection. It only configures the request, but the network activity only starts with the call of send.

**Syntax:** xhr.open(method, URL, [async, user, password])

Source: <https://javascript.info/xmlhttprequest#the-basics>

**Question:** **How do you open a connection and send a XMLHttpRequest to the server?**

**Interview Answer:** If we want to open a connection and send a XMLHttpRequest to a server we must invoke the send method to send the message body. Some request methods like GET do not have a body. And some of them like POST use body to send the data to the server.

**Syntax:** xhr.send([body])

Source: <https://javascript.info/xmlhttprequest#the-basics>

**Question:** **What are the three events most widely used in XMLHttpRequests?**

**Interview Answer:** The most widely events used in XMLHttpRequest are the load, error, and progress events. The load event happens when the request is complete (even if HTTP status is like 400 or 500), and the response is fully downloaded. The error event happens when the request could not be made, e.g., network down or invalid URL. The progress event triggers periodically while the response is being downloaded, reports how much has been downloaded.
...
**Question:** **What method is used to terminate a XMLHttpRequest?**

**Interview Answer:** We can terminate the request at any time. The call to xhr.abort() triggers the abort event, and xhr.status becomes 0 (unset state or has not been initialized).

Source: <https://javascript.info/xmlhttprequest#aborting-request>

**Question:** **What is the mode if the open() methods async parameter is set to false?**
...
**Question:** **Can you send custom headers via an XMLHttpRequest to a server?**

**Interview Answer:** XMLHttpRequest allows both to send custom headers and read headers from the response.

Source: <https://javascript.info/xmlhttprequest#http-headers>

**Question:** **What does the setRequestHeader method do?**

**Interview Answer:** The setRequestHeader sets the request header with the given name and value.
...
**Question:** **Does a XMLHttpRequest have any limitations in relation the Headers?**

**Interview Answer:** Several headers are managed exclusively by the browser, e.g., Referer and Host. XMLHttpRequest is not allowed to change them, for the sake of user safety and correctness of the request. Another peculiarity of XMLHttpRequest is that one cannot undo setRequestHeader. Once the header is set, it is set. Additional calls add information to the header, but do not overwrite it.
...
**Question:** **Example the function and syntax of the getResponseHeader() method?**
...
**Question:** **Can you explain the function and syntax of the getAllResponseHeader() method?**
...
**Question:** **What built-in object can we used to make a POST request?**

**Interview Answer:** To make a POST request, we can use the built-in FormData object. We can create it, optionally fill from a form, append more fields if needed.
...
**Question:** **Is there a way to track upload progress in relation to a XMLHttpRequest?**

**Interview Answer:** If we are uploading something big, then we are surely more interested in tracking the upload progress. But we should be aware that the xhr.onprogress doesn’t help here. The xhr.upload method is used to track upload progress. It generates events, similar to xhr, but xhr.upload triggers them solely on uploading.
...
**Question:** **How do cross-origin request function in relation to XMLHttpRequest?**

**Interview Answer:** XMLHttpRequest can make cross-origin requests, using the same CORS policy as fetch. Just like fetch, it does not send cookies and HTTP-authorization to another origin by default. To enable them, set xhr.withCredentials to true.
